...
#       - run with all data, and again with US projects' data excluded
...
open(my $fh, '>', 'data_out.txt');

print $fh "TYPE\tMERGE_TIME\tBWA_TIME\tDOWN_TIME\n";
...
while(<IN>) {
  ...
  if (defined ($json->{normal_specimen}{alignment}{timing_metrics})) {
    ...
    # print to file
    print $fh "NORM\t$norm_merge_timing_seconds\t$norm_bwa_timing_seconds\t$norm_download_timing_seconds\n";
    ...
    foreach my $tumor (@{$json->{aligned_tumor_specimens}}) {
      ...
      print $fh "TUMOR\t$tumor_merge_timing_seconds\t$tumor_bwa_timing_seconds\t$tumor_download_timing_seconds\n";
      # show progress
      print "."
